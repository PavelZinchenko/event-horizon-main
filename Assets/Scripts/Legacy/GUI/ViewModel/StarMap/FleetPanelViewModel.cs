...
namespace ViewModel
{
	public class FleetPanelViewModel : MonoBehaviour
	{
		...
		private void UpdateActiveShips(bool isEditorMode)
		{
            ...
            if (isEditorMode)
                ActiveShipsGroup.transform.InitializeElements<ShipInfoViewModel, IShip>(_database.ShipBuildList.OrderBy(build => build.Ship.Id.Value).Select<ShipBuild,IShip>(build => new EditorModeShip(build, _database)), UpdateShip);
            else if (_playerFleet != null)
			    ActiveShipsGroup.transform.InitializeElements<ShipInfoViewModel, IShip>(_playerFleet.ActiveShipGroup.Ships, UpdateShip);
		}
