...
namespace StarSystem
{
	...
	public class OrbitalObjects : MonoBehaviour
	{
		...
		{
			planetObject.Image.sprite = _planetSettings.GetPlanetImage(model.Type, model.Seed);
		...
		private void CreateBoss(Galaxy.StarContent.Boss.Facade boss, Color color)
		{
			...
			var flagship = fleet.Ships.FirstOrDefault(item => item.Model.ShipType == ShipType.Flagship) ?? fleet.Ships.First();

			var random = new System.Random(boss.GetHashCode());
			_lastAngle += 60 + random.Next(240);
			var position = RotationHelpers.Direction(_lastAngle)*_orbitRadius;            
			ships.CreateShips(fleet.Ships.Where(item => item.Model.ShipType != ShipType.Flagship), position);
