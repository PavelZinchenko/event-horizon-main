...
namespace Combat.Domain
{
    public class FleetModel : IFleetModel
    {
        public FleetModel(IEnumerable<IShip> ships, UnitSide unitSide, IDatabase database, int level, PlayerSkills playerSkills = null)
        {
            ...
            {
                var shipSpec = playerSkills != null ? BuildSpecAndApplySkills(ship, playerSkills, settings) : ship.CreateBuilder().Build(settings);
        ...
        private static IShipSpecification BuildSpecAndApplySkills(IShip ship, PlayerSkills skills, GameDatabase.DataModel.ShipSettings settings)
        {
