...
namespace Session.Model
{
	public class StarMapData : IDataChangedCallback
	{
		...
		public StarMapData(SessionDataReader reader, IDataChangedCallback parent)
		{
			...
			planetDataItemCount = reader.ReadInt(EncodingType.Plain);
			_planetData = new ObservableMap<long, int>(this);
			for (int i = 0; i < planetDataItemCount; ++i)
			{
				long key;
				int value;
				key = reader.ReadLong(EncodingType.Plain);
				value = reader.ReadInt(EncodingType.Plain);
		...
		public void Serialize(SessionDataWriter writer)
		{
				...
				writer.WriteInt(_planetData.Count, EncodingType.Plain);
				foreach (var item in _planetData.Items)
				{
					writer.WriteLong(item.Key, EncodingType.Plain);
					writer.WriteInt(item.Value, EncodingType.Plain);
				}
